...
class Parser {
  ...
  expect(type, value) {
    ...
    if (t.type !== type || (value !== undefined && t.value !== value)) {
      throw new Error(`Expected ${type} "${value || ''}" got ${t.type} "${t.value}" at token ${this.pos}`);
    }
    return t;
  }
  ...
  parseStatement() {
    ...
    if (t.type === TOKEN_TYPES.PUNCTUATION && t.value === ':' &&
        this.tokens[this.pos + 1] && this.tokens[this.pos + 1].type === TOKEN_TYPES.PUNCTUATION &&
  ...
  parseReturn() {
    ...
    if (t.type === TOKEN_TYPES.EOF ||
        t.type === TOKEN_TYPES.PUNCTUATION && t.value === ';' ||
        (t.type === TOKEN_TYPES.KEYWORD && ['end', 'else', 'elseif', 'until'].includes(t.value))) {
  ...
  parseExpressionStatement() {
    ...
    const compoundOps = ['+=', '-=', '*=', '/=', '%=', '^=', '..=', '//='];
    for (const cop of compoundOps) {
      if (this.match(TOKEN_TYPES.OPERATOR, cop)) {
        const baseOp = cop.slice(0, -1);
  ...
  parsePrimaryExpression() {
    const t = this.peek();
    if (t.type === TOKEN_TYPES.NUMBER) return { type: 'NumberLiteral', value: this.advance().value };
    if (t.type === TOKEN_TYPES.STRING) return { type: 'StringLiteral', value: this.advance().value };
    if (t.type === TOKEN_TYPES.KEYWORD && t.value === 'true') { this.advance(); return { type: 'BooleanLiteral', value: true }; }
    if (t.type === TOKEN_TYPES.KEYWORD && t.value === 'false') { this.advance(); return { type: 'BooleanLiteral', value: false }; }
    if (t.type === TOKEN_TYPES.KEYWORD && t.value === 'nil') { this.advance(); return { type: 'NilLiteral' }; }
    if (t.type === TOKEN_TYPES.KEYWORD && t.value === 'function') { this.advance(); return this.parseFunctionBody(); }
    if (t.type === TOKEN_TYPES.PUNCTUATION && t.value === '{') return this.parseTableConstructor();
    if (t.type === TOKEN_TYPES.PUNCTUATION && t.value === '(') {
      this.advance();
    ...
    if (t.type === TOKEN_TYPES.IDENTIFIER) return { type: 'Identifier', name: this.advance().value };
    if (t.type === TOKEN_TYPES.PUNCTUATION && t.value === '.') {
      this.advance(); this.expect(TOKEN_TYPES.PUNCTUATION, '.'); this.expect(TOKEN_TYPES.PUNCTUATION, '.');
      return { type: 'VarargExpression' };
    }
    throw new Error(`Unexpected token: ${t.type} "${t.value}" at position ${this.pos}`);
  }
    ...
    while (this.peek().type !== TOKEN_TYPES.PUNCTUATION || this.peek().value !== '}') {
      if (this.peek().type === TOKEN_TYPES.PUNCTUATION && this.peek().value === '[') {
        ...
        fields.push({ type: 'IndexedField', key, value });
                 ...
                 this.tokens[this.pos + 1].type === TOKEN_TYPES.OPERATOR &&
                 this.tokens[this.pos + 1].value === '=') {
